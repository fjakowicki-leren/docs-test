        ...
        <div id="filters" class="visible-when-content-ready" data-store="filters-nav">
            {% for product_filter in product_filters %}
                {% if product_filter.type == 'price' %}
                    ...
                    {{ component(
                        'price-filter',
                        {'group_class': 'price-filter-container ' ~ price_container_classes, 'title_class': 'font-md-small font-weight-bold mt-1 mb-3 text-uppercase', 'button_class': 'btn btn-default ' ~ price_btn_classes }
                    ) }}
                                ...
                                <a href="#" class="js-accordion-toggle font-md-small text-uppercase row no-gutters align-items-center py-md-2" data-accordion-target="{{ product_filter.key }}">
                                    ...
                                    <div class="col-auto my-1">
                                        <span class="js-accordion-toggle-inactive">
                                          {% include "snipplets/svg/chevron-right.tpl" with {svg_custom_class: "icon-inline svg-icon-text icon-lg font-md-body mr-1"} %}
                                        </span>
                                        <span class="js-accordion-toggle-active" style="display: none;">
                                          {% include "snipplets/svg/chevron-down.tpl" with {svg_custom_class: "icon-inline svg-icon-text icon-lg font-md-body"} %}
                                        </span>
                                    </div>
                                </a>
                            </div>
